...
const Post = () => {
  ...
  const handleClickNavigateCancel = () => {
    navigate("/home");
  };
  ...
  const handleSubmit = useMutation(async (e) => {
    try {
      ...
      if (form.image1 == "" && form.image2 == "" && form.image3 == "" && form.image4 == "" && form.image5 == "") {
        setAlert(true);
        setAlertMessage("Create post failed! Please upload at least 1 image.");
      } else if (form.description == "" && form.title == "") {
        setAlert(true);
        setAlertMessage("Create post failed! Title and description cannot be empty!");
      } else {
    ...
    <>
      ...
      <form onSubmit={(e) => handleSubmit.mutate(e)} className="flex w-full py-24 px-20">
        ...
        <div className="w-[40%] flex flex-col items-center">
          <div className="w-[70%]">
            <input onChange={handleChangeForm} type="text" name="title" className="text-lg py-2 px-3 w-full rounded bg-[#E7E7E7] border-2 border-[#2FC4B2]" placeholder="Title" />
            <textarea onChange={handleChangeForm} name="description" className="text-lg py-2 px-3 mt-6 w-full h-40 rounded bg-[#E7E7E7] border-2 border-[#2FC4B2]" placeholder="Description"></textarea>
          </div>
          <div className="mx-auto mt-10">
            <button onClick={handleClickNavigateCancel} type="button" className="py-2 px-8 rounded-md text-sm font-medium text-black hover:bg-[#bdbbbb] bg-[#E7E7E7]">
              Cancel
            </button>
            <button type="submit" className="py-2 px-8 ml-5 rounded-md text-sm font-medium hover:bg-[#1a9b8c] text-white bg-[#2FC4B2]">
              Post
            </button>
          </div>
        </div>
